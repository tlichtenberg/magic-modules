...
~> **NOTE on `google_sql_database_instance`:** - Cloud SQL instances include a
...
resource "google_sql_database_instance" "master" {
  ...
  settings {
    # Cloud SQL instance tiers are based on the machine
...
~> **NOTE:** Read replicas are not suppored for Cloud SQL for SQL Server. 
...
* `failover_target` - (Optional - applies only to Legacy High Availabilty, which will be deprecated in July 2020)
